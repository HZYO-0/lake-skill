"""Data readiness checks for lake-skill doctor."""
...
def check_privacy_risk(messages: list[Message]) -> dict:
    ...
    if has_redacted == total:
        ...
        return {"name": "隐私风险", "passed": False, "summary": "未脱敏", "detail": "消息未脱敏，上传前请运行 lake-skill redact"}
...
def format_readiness_report(results: dict) -> str:
    ...
    if overall == "ready":
        lines.append("数据充分，建议运行完整 LakeSkill 分析流程。")
