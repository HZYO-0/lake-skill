...
[English](README_EN.md) | [安装指南](INSTALL.md) | [快速开始](docs/quickstart.md) | [聊天记录准备](docs/chat_record_preparation.md) | [迁移说明](docs/migration_from_bondlens.md)
...
LakeSkill 湖镜是一个面向 Codex、Claude Code、OpenCode 等 agent runtime 的亲密关系聊天分析 Skill，并提供可选的本地预处理 CLI。它不扮演 TA，不替你猜测 TA 的真心，也不制造情绪依赖。它更像一面湖：把聊天记录、关键事件和你的感受轻轻铺开，让关系里的信号、涟漪、误会和边界变得可以被看见。

湖水像镜子。风停下来的时候，它会照出岸边，也照出人心。亲密关系里很多痛苦，来自我们在涟漪最大的时候急着判断：TA 是不是不在乎，我是不是做错了，这段关系是不是没有希望。LakeSkill 先帮你把证据分层，再把信号放回时间线里，让判断慢一点，也更稳一点。
...
所以 LakeSkill 不问“怎样让 TA 离不开我”。它问的是：证据支持什么，不支持什么；现在适合靠近、稳定、修复，还是先回到自己。
...
## LakeSkill 和 ex-skill / colleague-skill 有什么不同

Skill 社区里有很多优秀项目在做 person distillation 或 persona simulation，例如把前任、同事、角色或某类人“蒸馏”为可对话对象。LakeSkill 走的是另一条线：不扮演 TA，而是解释证据。

| 类型 | 典型目标 | 典型输出 | LakeSkill 的区别 |
|---|---|---|---|
| dot-skill / colleague-style | 把某个人、同事或角色知识打包成 Skill | 像某个人一样反馈、吐槽、复盘或陪聊 | LakeSkill 不模拟某个人的声音，只分析聊天证据支持什么 |
| ex-skill-style | 把前任或过去关系蒸馏成可交互 persona | 情绪化回看、对话模拟、记忆式互动 | LakeSkill 不复活关系对象，不预测 TA 的真实内心 |
| LakeSkill | 把聊天记录变成行动判断 | 行动卡、证据报告、消息草稿、审计说明 | 核心是 evidence interpretation + action coaching |

一句话定位：别让 AI 猜 TA 爱不爱你。让 LakeSkill 告诉你：聊天证据支持什么、不支持什么、下一步怎么做更稳。
...
如果证据不足，LakeSkill 应该明确降级：只输出低置信度局部观察，不生成完整人格或关系判断。
